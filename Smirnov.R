...
# Aplicar o teste de Kolmogorov-Smirnov para cada metodologia com jitter
resultado_ks <- dados %>%
  group_by(Metodologia) %>%
  summarise(
    ks_test = list(ks.test(adicionar_jitter(Erros), "pnorm", mean = mean(Erros), sd = sd(Erros))), # Aplica o teste KS com jitter
    .groups = 'drop'
  )

# Extrair o valor p do teste de Kolmogorov-Smirnov e arredondar para 2 casas decimais
resultado_ks <- resultado_ks %>%
  mutate(p_value = round(sapply(ks_test, function(x) x$p.value), 2)) %>%
  select(Metodologia, p_value)

# Converter a tabela de resultados em uma imagem usando gridExtra
tabela <- tableGrob(resultado_ks, rows = NULL)

# Salvar a tabela como imagem .png
caminho_imagem <- "/home/henriquemedeiros/Documents/TCC/2024.3 TCC/R_Plotagens/graficos/resultado_ks.png"
